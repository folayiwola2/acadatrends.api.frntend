...
import { createQuestionCategory } from '../store/actions/projectActions'
import swal from 'sweetalert';
import '../layouts/swal.css'

class AddQuestionCategory extends Component {
    ...
    handleSubmit = async (e) => {
        ...
        let obj = { name, code };
        await this.props.createQuestionCategory(obj);
        console.log("handle", this.state)
        let props = this.props;
        if (props.status === "Success") {
            setTimeout(() => {
                // this.setState({ name: "", code: "", isLoading: false, category: props.status });
                swal("Response", "Question Category Created Successfully", "success");
                this.props.history.push('/admin/view-question-category')
            }, 2500)
        } else {
            setTimeout(() => {
    ...
    return {
        createQuestionCategory: (category) => {
            return new Promise((resolve) => {
                dispatch(createQuestionCategory(category, (res) => {
                    resolve(res);
                }))
            })
        }
...
export default connect(mapStateToProps, mapDispatchToProps)(AddQuestionCategory)
