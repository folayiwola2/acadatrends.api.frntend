...
class EditQuestionCategory extends Component {
    ...
    componentDidMount() {
        ...
        fetch(`${base_url}/questionCategory/${id}`).then((response) => {
    ...
    handleSubmit = (e) => {
        ...
        fetch(`${base_url}/questionCategory/${id}`, {
            ...
            return console.log("Update response", response)
            if (response.status === 200) {
                swal("Response", "Successfully updated Category", "success");
                setTimeout(this.props.history.push('/admin/view-question-category'), 2000)
